...
type Tab = "score" | "gaps" | "bullets" | "cover";
...
export default function ResultsPage() {
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#111614", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" }}>
        <div style={{ width: 36, height: 36, border: "2px solid #EF9F27", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite", margin: "0 auto 16px" }} />
        <p style={{ color: "#6b8a7a", fontSize: 14 }}>Loading your analysis...</p>
      </div>
      <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
    </div>
  );
  ...
  const verdictColor = match_score.verdict === "Strong Match" ? "#5DCAA5"
    : match_score.verdict === "Partial Match" ? "#fb923c" : "#f87171";
  ...
  const bgCard = "#181f1c";
  const text = "#e8f0ec";
  const textMuted = "#6b8a7a";
  const textDim = "#3d5a4e";
  const orange = "#EF9F27";
  ...
  return (
    <div style={{ background: bg, minHeight: "100vh", fontFamily: "system-ui,-apple-system,sans-serif", color: text }}>
      ...
      {/* Header */}
      <header style={{ borderBottom: `1px solid ${border}`, position: "sticky", top: 0, background: bg, zIndex: 10 }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <button onClick={() => router.push("/")} style={{ background: "none", border: "none", color: textMuted, cursor: "pointer", fontSize: 14 }}>
            ← New Analysis
          </button>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" as const, justifyContent: "flex-end" }}>
            <span style={{ fontSize: 13, color: textDim }}>
              {job.title || "Analysis"}{job.company ? ` · ${job.company}` : ""}
            </span>
            <span style={{ fontSize: 11, fontFamily: "monospace", background: "rgba(239,159,39,0.12)", color: orange, padding: "3px 8px", borderRadius: 6 }}>
              {result.processing_time_ms}ms
            </span>
            <button
              onClick={handleDownloadDocx}
              disabled={downloading}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 8, border: `1px solid ${border}`, background: downloading ? "#1e2a26" : "#161f1c", color: downloading ? textDim : text, fontSize: 12, fontWeight: 500, cursor: downloading ? "not-allowed" : "pointer", fontFamily: "inherit", transition: "all 0.15s" }}
              onMouseEnter={(e) => { if (!downloading) e.currentTarget.style.borderColor = teal; }}
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px 80px" }}>
        ...
        {/* Summary bar */}
        <div style={{ background: bgCard, border: `1px solid ${border}`, borderRadius: 20, padding: "28px 32px", marginBottom: 24, display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap" as const, gap: 20 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 12 }}>
              <span style={{ fontSize: 56, fontWeight: 700, fontFamily: "monospace", color: orange, lineHeight: 1 }}>
                {match_score.overall}
              </span>
              <div>
                <div style={{ fontSize: 15, fontWeight: 600, color: verdictColor, marginBottom: 2 }}>{match_score.verdict}</div>
                <div style={{ fontSize: 12, color: textDim }}>overall match score</div>
              </div>
            </div>
            <p style={{ fontSize: 14, color: textMuted, lineHeight: 1.6, maxWidth: 520 }}>{match_score.summary}</p>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, textAlign: "right", flexShrink: 0 }}>
            <div style={{ fontSize: 13, color: textDim }}><span style={{ color: "#f87171", fontWeight: 600 }}>{gap_analysis.critical_gaps_count}</span> critical gaps</div>
            <div style={{ fontSize: 13, color: textDim }}><span style={{ color: "#5DCAA5", fontWeight: 600 }}>{gap_analysis.present_skills.length}</span> skills matched</div>
            <div style={{ fontSize: 13, color: textDim }}><span style={{ color: orange, fontWeight: 600 }}>{bullet_rewrites.total_bullets}</span> bullets rewritten</div>
          </div>
        </div>

        {/* Tabs */}
        <div style={{ display: "flex", gap: 4, padding: 4, background: bgCard, border: `1px solid ${border}`, borderRadius: 14, width: "fit-content", marginBottom: 24, flexWrap: "wrap" as const }}>
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              style={{ padding: "8px 18px", borderRadius: 10, border: "none", fontSize: 13, fontWeight: 500, cursor: "pointer", background: activeTab === tab.id ? (tab.id === "cover" ? orange : teal) : "transparent", color: activeTab === tab.id ? (tab.id === "cover" ? "#000" : "#fff") : textMuted, transition: "all 0.15s", fontFamily: "inherit" }}
            >
              {tab.label}
            </button>
          ))}
        ...
        {activeTab === "gaps"    && <GapAnalysis gapAnalysis={gap_analysis} result={result} />}
        {activeTab === "bullets" && <BulletComparison bulletRewrites={bullet_rewrites} />}
        {activeTab === "cover"   && <CoverLetter result={result} />}
      </div>
    </div>
  );
}